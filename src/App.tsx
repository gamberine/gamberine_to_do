...
import LogoRed from "./assets/logo-gamberine-red.svg"; // ou: ...svg?url
...
const Icon = {
  ...
  Search: (p: React.SVGProps<SVGSVGElement>) => (
    <svg viewBox="0 0 24 24" fill="none" className="searchIcon" stroke="currentColor" strokeWidth="2" {...p}>
...
const classes = {
  ...
  field: "rounded-xl px-4 py-3 placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-[var(--red)] focus:bg-white/[.06]",
};
    ...
    <div className={classes.app}>
      <div className={classes.container}>
        ...
        <header className={`${classes.glass} rounded-3xl p-5 md:p-7 flex flex-col md:flex-row md:items-center md:justify-between gap-4 glass-glow`}>
          ...
          {/* <h1 className="text-3xl font-extrabold">Gamberine To-Do</h1> */}
          <div className="flex items-center gap-4">
            <img
              src={LogoRed}
              alt="Gamberine To-Do"
              className="h-12 w-12 rounded-2xl ring-2 ring-[var(--red)] bg-black object-contain p-1"
            />
            <div>
              <h1 className="text-2xl md:text-3xl font-extrabold tracking-tight">Gamberine To-Do</h1>
              <p className="text-sm text-zinc-400">Foco em organização pessoal e profissional</p>
            </div>
          </div>

          <div className="searchForm flex flex-1 items-center gap-3 md:max-w-xl">
            <div className={`flex items-center gap-2 flex-1 ${classes.field}`}>
              <Icon.Search className="searchIcon h-4 w-4 opacity-70" />
        ...
        {/* Layout */}
        <div className="mt-6 flex xl:flex-row lg:flex-row md:flex-col sm:flex-col flex-col md:items-center md:justify-between gap-6 sm:gap-8">
        
            
          {/* Sidebar */}
          <aside className={`${classes.glass} rounded-3xl p-5 h-fit xl:sticky top-6`}>
            ...
            <div className="mt-6 pt-6 border-t border-white/10">
              <button onClick={clearCompleted} className={`${classes.btn} w-full justify-center bg-white/5 border border-white/10 hover:bg-white/10`}>
          ...
          <main className="flex flex-col gap-4">
            ...
            <section className="gap-3 columns-1 sm:columns-2 xl:columns-2">
              ...
              {filtered.map(task => {
                ...
                return (
                  ...
                  <article
                    key={task.id}
                    className={`${classes.glass} [break-inside:avoid] mb-6 rounded-3xl p-4 md:p-5 transition hover:bg-white/[.06] ${overdue ? "outline outline-1 outline-red-500/40" : ""}`}
                  >

                    <div className="flex lg:flex-row lg:flex-row flex-col items-start gap-3 md:flex-col md:items-center">
                      <input type="checkbox" checked={task.completed} onChange={() => toggle(task.id)} title={`Mark "${task.title}" as ${task.completed ? "incomplete" : "complete"}`} aria-label={`Mark "${task.title}" as ${task.completed ? "incomplete" : "complete"}`} className="mt-1 h-5 w-5 accent-[var(--red)]" />
                        ...
                        <h3 className={`text-lg font-semibold ${task.completed ? "line-through text-zinc-500" : ""}`}>{task.title}</h3>
